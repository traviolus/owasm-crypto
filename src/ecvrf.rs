...
pub fn scalar_multiply(p: &(Mpz, Mpz), scalar: &Mpz) -> Option<(Mpz, Mpz)> {
    ...
    let mut q = scalar_multiply(p, &scalar.div_floor(&Mpz::from(2))).unwrap();
    q = edwards_add(&q, &q);
    if scalar & Mpz::one() == Mpz::one() {
        q = edwards_add(&q, p);
    }
    Some(q)
}
...
pub fn ecvrf_decode_proof(pi: &[u8]) -> Option<((Mpz, Mpz), Mpz, Mpz)> {
    ...
    let c_pi: Vec<u8> = Vec::from(&pi[32..48]).into_iter().rev().collect();
    let s_pi: Vec<u8> = Vec::from(&pi[48..]).into_iter().rev().collect();
...
pub fn ecvrf_hash_to_curve_elligator2_25519(y: &[u8], alpha: &[u8]) -> Option<Vec<u8>> {
    ...
    let e2 = match gx1.powm(&(Mpz::from(&*PRIME - Mpz::one()) >> 1), &*PRIME).to_str_radix(10).as_str() {
        "0" => true,
        "1" => true,
        _ => false,
    };
    ...
    let edwards_y = modulus(
        &(Mpz::from(x.clone() - 1) * inverse(&(x.clone() + 1))),
        &*PRIME,
    );
    ...
    let e3 = u8::from_str_radix(Mpz::from(&y_coordinate & Mpz::one()).to_str_radix(10).as_str(), 10).unwrap() == 1u8;
    if e2 ^ e3 {
...
pub fn ecvrf_hash_points(
    ...
    let truncated_c_string: Vec<u8> = Vec::from(&c_string[0..16]).into_iter().rev().collect();
